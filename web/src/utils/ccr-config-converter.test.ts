...
const openaiToAnthropic = convertCCRConfigToChannel(openaiConfig, 'anthropic', 'kyxcode', '1968121800');
const anthropicResult = JSON.parse(openaiToAnthropic);
console.log('转换结果：', openaiToAnthropic);
console.log('Transformer 正确性检查：',
  JSON.stringify(anthropicResult.transformer) === '{"use":["Anthropic"]}' ? '✅ 正确' : '❌ 错误'
);

console.log('\n2. 测试 Anthropic -> Gemini 转换：');
const anthropicToGemini = convertCCRConfigToChannel(anthropicConfig, 'gemini', 'kyxcode', '1968121800');
...
console.log('- 模型级 reasoning：',
  openaiResult.transformer['gemini-2.5-pro'] &&
  openaiResult.transformer['gemini-2.5-flash'] ? '✅ 正确' : '❌ 错误'
);
...
console.log('模型保持一致:', JSON.stringify(originalModels) === JSON.stringify(convertedModels) ? '✅ 正确' : '❌ 错误');
...
const complexToAnthropic = convertCCRConfigToChannel(complexOpenaiConfig, 'anthropic', 'test', 'test');
