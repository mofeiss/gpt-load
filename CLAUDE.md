...
**⚠️ 端口管理警告**
- 当前项目在 Docker 中运行，使用端口 3001
- 绝对不能执行 `lsof -ti :3001 | xargs kill -9` 命令
...
- 无论任何时候都不能执行  “lsof -ti :3001 | xargs kill -9”  这会完全杀死我的docker进程正在运行的重要服务，包括我和你的API通信都是基于docker服务，你结束了进程就会中断我们的连接
- 当前项目你只能构建和编译检查错误，绝对不能运行该项目，如果写完了代码需要调试验证修改是否生效，请将这个任务交给用户，立刻停止输出